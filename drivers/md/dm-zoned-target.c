			   ...
			   struct bio *bio)
{
    ...
    /*
     * dmz_lock_metadata 는 metadata 가 수정되는 동안에 metadata가 flush가 실행되지
     * 않도록 하는 함수이다. lock/unlock metadata access. 이것은 map lock and zone state
     * management ( active and reclaim state are mutually exclusive ) 를 통해 달성된다.
     */
